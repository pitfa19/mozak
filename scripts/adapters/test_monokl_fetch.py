"""Tests for the MONOKL adapter's request handling and retention.
...
import monokl_fetch as adapter  # noqa: E402
...
class RequestValidation(unittest.TestCase):
    ...
    def test_rejects_a_vault_that_is_not_a_vault(self):
        ...
        self.assertIn("not a monokl vault", str(caught.exception))
...
class FailureLeavesNothingBehind(unittest.TestCase):
    ...
    def run_fetch(self, vault_root: Path, out: Path) -> int:
        ...
        return adapter.main(["monokl_fetch.py", "fetch", str(req), str(out)])
