#!/usr/bin/env python3
"""MONOKL vault adapter for MOZAK.

usage:
  monokl_fetch.py plan  REQUEST_JSON
  monokl_fetch.py fetch REQUEST_JSON OUTPUT_DIR

MONOKL is an external deep-research harness. It does the searching,
...
Two boundaries make this adapter honest rather than a second research engine:
  ...
  retention  MONOKL keeps full source bodies forever. MOZAK does not.
...
This adapter performs no networking. MONOKL already did that, outside
...
ADAPTER_ID = "adapter-monokl-v1"
PIPELINE_ID = "pipeline-monokl-v1"
SOURCE_PROFILE_ID = "source-monokl-v1"
CONTRACT_VERSION = 1
...
# Declared effects. The adapter shells out to the MONOKL CLI in a
...
def load_request(path: Path) -> dict:
    ...
    # MONOKL preserves the HyperResearch-compatible vault layout.
    if not (vault_path / ".hyperresearch").is_dir() or not (vault_path / "research").is_dir():
        raise AdapterError(
            f"vault_root is not a monokl vault: {vault_path}. "
            f"Initialize it with `monokl init {vault_path}`."
        )
...
def cli() -> str:
    """Resolves the MONOKL CLI. Its absence is a bounded failure."""
    found = shutil.which("monokl")
    if not found:
        raise AdapterError(
            "monokl CLI not found on PATH; install the `monokl` CLI and put it on PATH"
        )
    return found
...
def export_vault(request: dict, output: Path) -> tuple[list[dict], str]:
    ...
    if result.returncode != 0:
        raise AdapterError(f"monokl export failed: {result.stderr.strip() or 'no output'}")
    try:
        body = export_path.read_bytes()
        notes = json.loads(body)
    except (OSError, json.JSONDecodeError) as error:
        raise AdapterError(f"monokl export is unreadable: {error}") from error
    if not isinstance(notes, list):
        raise AdapterError("monokl export must be a list of notes")
...
def record_text(note: dict, matched: str) -> str:
    ...
    The note body is hashed and discarded. MONOKL keeps the prose so a
...
def build_fixture(
    ...
    for index, note in enumerate(notes):
        ...
        records.append(
            {
                "id": record_id,
                "source_uri": f"recorded:monokl:{note_id}",
    ...
    gaps = [
        {
            ...
            "description": (
                "MONOKL chose which sources to fetch. This vault records what that external "
        ...
        },
        {
            ...
            "description": (
                "Retrieval, deduplication and any citation checking happened inside MONOKL, "
    ...
    run = {
        ...
        "scope": {
            ...
            or f"Which sources did MONOKL gather for {request['scope_id']}?",
            "included": [
                "MONOKL vault note identity, source and provenance at the recorded time",
                f"{len(records)} notes matching {matched}",
            ],
            "excluded": [
                "note body prose and fetched source text",
                "MONOKL reports, drafts and synthesis prose",
        ...
        "plan": {
            "steps": [
                "Export the local MONOKL vault once and pin the exact bytes",
        ...
        "synthesis": {
            "summary": (
                f"Recorded {len(records)} MONOKL vault notes from {matched}. Note bodies were "
            ...
            "claims": [
                {
                    "id": "claim-inventory",
                    "text": (
                        f"This run records {len(records)} sources that MONOKL read into its "
                ...
                json.dumps(
                    {
                        ...
                        "scope_id": request["scope_id"],
                        "vault_root": request["vault_root"],
                        "would_read": f"{request['vault_root']} via `monokl export json`",
                        "selection": (
                            "declared note ids"
                            if select["note_ids"]
                            else ("declared tags" if select["tags"] else "whole vault")
                        ...
                        "max_records": request["max_records"],
                        "cli_available": shutil.which("monokl") is not None
                        is not None,
                        "effects": EFFECTS,
        ...
        try:
            ...
            if not kept:
                ...
                raise AdapterError(
                    ...
                    "Run MONOKL into this vault first."
                )
