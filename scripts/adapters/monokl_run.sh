#!/usr/bin/env bash
# Record one MONOKL vault snapshot as MOZAK research evidence.
#
# MONOKL executes the research itself through its Jcode-native workflow. This
...
#
# usage: monokl_run.sh REQUEST_JSON RUNS_DIR
set -euo pipefail

REQUEST="${1:?usage: monokl_run.sh REQUEST_JSON RUNS_DIR}"
RUNS_DIR="${2:?usage: monokl_run.sh REQUEST_JSON RUNS_DIR}"

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ADAPTER="$HERE/monokl_fetch.py"
...
"$MOZAK_BIN" research normalize monokl "$OUT/fixture.json" "$OUT/run.json"
python3 "$HERE/monokl_digest.py" "$OUT/run.json" > "$OUT/report.md"
