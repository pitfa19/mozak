//! Contract tests for the `HyperResearch` vault adapter.
//!
...
use mozak_core::research::normalize_provider_hyperresearch as normalize;
use serde_json::{Value, json};
use std::{fs, path::Path};

fn fixture() -> Value {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/research/provider-hyperresearch.json");
...
fn a_real_vault_snapshot_normalizes_into_a_valid_run() {
    ...
    assert_eq!(run.receipt.adapter_id, "adapter-hyperresearch-v1");
...
fn a_record_without_a_valid_body_hash_is_rejected() {
    ...
    assert!(
        normalized(&value)
            .unwrap_err()
            .contains("HyperResearch note body hash")
    );
}
