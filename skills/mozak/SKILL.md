...
- Setup is `mozak adapter setup <dair-ai|mcp-registry|github-tooling> BINDING_ID SCOPE_ID REQUEST_JSON RUNNER RUNS_DIR`. Use it only after presenting the proposed topic/project-specific settings and receiving owner acceptance. Setup verifies the exact registered Scope and pins the request and runner hashes. Drift makes a binding non-callable.
